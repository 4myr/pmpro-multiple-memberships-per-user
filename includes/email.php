...
function pmprommpu_send_checkout_emails($user_id, $checkout_id = -1) {
	...
	if($checkout_id>0) {
		...
		$pmproemail->subject = sprintf(__("تأیید عضویت شما در %s", "pmpro"), get_option("blogname"));
...
// 				$pmproemail->template = "checkout_trial";
			...
			if($invoice->getDiscountCode())
				$pmproemail->data["discount_code"] = "<p>" . __("کد تخفیف", "pmpro") . ": " . $invoice->discount_code->code . "</p>\n";
			else
		...
		elseif(pmpro_areLevelsFree($levels))
		{
			...
			if(!empty($discount_code))
				$pmproemail->data["discount_code"] = "<p>" . __("کد تخفیف", "pmpro") . ": " . $discount_code . "</p>\n";		
			else
				$pmproemail->data["discount_code"] = "";		
		}						
		else
		...
		$pmproemail->subject = sprintf(__("تکمیل عضویت در %s", "pmpro"), get_option("blogname"));
